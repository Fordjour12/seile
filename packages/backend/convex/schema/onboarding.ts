...
export const onboardingStateTable = defineTable({
  ...
  currentPhase: onboardingPhaseValidator,
  dayNumber: v.number(),
  startedAt: v.number(),
...
export const activityAssignmentsTable = defineTable({
  ...
  assignedAt: v.number(),
  dueAt: v.number(),
}).index("by_userId", ["userId"]).index("by_userId_day", ["userId", "dayNumber"]);
...
export const activityEventsTable = defineTable({
  ...
  createdAt: v.number(),
}).index("by_assignmentId", ["assignmentId"]).index("by_userId", ["userId"]);
...
export const signalsTable = defineTable({
  ...
  metadata: v.optional(v.any()),
  createdAt: v.number(),
}).index("by_userId", ["userId"]).index("by_userId_category", ["userId", "category"]);
...
}).index("by_userId", ["userId"]).index("by_userId_shownAt", ["userId", "shownAt"]);
...
export const feedbackTable = defineTable({
  ...
  createdAt: v.number(),
}).index("by_suggestionId", ["suggestionId"]).index("by_userId", ["userId"]);
