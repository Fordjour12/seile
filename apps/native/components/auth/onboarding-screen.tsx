...
import {
  ...
  type OnboardingAiTone,
  type OnboardingDomainId,
  type OnboardingNotificationSettings,
  type OnboardingPlanningStyle,
  type OnboardingStep,
...
const TONE_MESSAGES: Record<OnboardingAiTone, string> = {
  direct:
...
export function OnboardingScreen() {
  ...
  const [domains, setDomains] = useState<OnboardingDomainId[]>([
    "faith",
    "career",
    "finance",
    "health",
  ]);
  const [pinnedDomainIds, setPinnedDomainIds] = useState<OnboardingDomainId[]>([
    "faith",
  ]);
  const [planningStyle, setPlanningStyle] =
    useState<OnboardingPlanningStyle>("balanced");
  const [aiTone, setAiTone] = useState<OnboardingAiTone>("direct");
  const [notifications, setNotifications] = useState<OnboardingNotificationSettings>({
  ...
  useEffect(() => {
    ...
    void loadOnboardingDraft().then((draft) => {
      ...
      setPinnedDomainIds(draft.pinnedDomainIds);
      setPlanningStyle(draft.planningStyle);
  ...
  function toggleDomain(id: OnboardingDomainId) {
    setDomains((current) => {
  ...
  function togglePinnedDomain(id: OnboardingDomainId) {
            ...
            <View style={{ gap: 8 }}>
              ...
              >
                ...
                <Text
                  ...
                  style={{ color: theme.foreground, lineHeight: 20 }}
                >
                  You&apos;ll move through a guided first-run preview and finish
                  onboarding before entering the main finance workspace.
                </Text>
              </Card>
              <Button
