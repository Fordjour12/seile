...
export const sendPlannerChatMessage = action({
  ...
  handler: async (ctx, args) => {
    ...
    const contextBefore = await ctx.runQuery(
      internalApi["productivity/planner/queries"].getPlannerAgentContext,
      { userId },
    );
    ...
    const commandReservation = await ctx.runMutation(
      internalApi["productivity/planner/mutations"].reservePlannerChatCommand,
      {
        userId,
        threadId,
        messageKey,
    ...
    if (commandReservation.command?.status === "completed") {
      ...
      await ctx.runMutation(internalApi["productivity/planner/mutations"].completePlannerChatCommand, {
        userId,
        messageKey,
    ...
    const contextAfter = await ctx.runQuery(internalApi["productivity/planner/queries"].getPlannerAgentContext, {
      userId,
    });
...
export const runWeeklyReviewCycle = internalAction({
  ...
  handler: async (ctx) => {
    ...
    const states = await ctx.runQuery(internalApi["productivity/planner/queries"].listAgentEnabledStates, {});
    let reviewedCount = 0;
    ...
    for (const state of states) {
      ...
      try {
        const targetPlan = await ctx.runQuery(
          internalApi["productivity/planner/queries"].getLatestPastWeeklyPlanWithoutReview,
          { userId: state.userId },
        );
      ...
      try {
        ...
        if (result.object.shouldAdjust) {
          const replanResult = await ctx.runMutation(
            internalApi["productivity/planner/mutations"].performAgentReplan,
            {
              userId: state.userId,
  ...
  handler: async (ctx) => {
    ...
    for (const state of states) {
      try {
        ...
        await ctx.runMutation(internalApi["productivity/planner/mutations"].updateAgentBurnoutState, {
          userId: state.userId,
...
async function draftWeeklyPlanForUser(
  ...
  return await ctx.runMutation(internalApi["productivity/planner/mutations"].storeAgentWeeklyPlan, {
    userId: input.userId,
    weekStart: context.week.startDate,
...
async function reviewWeeklyPlanForUser(
  ...
  ensurePlannerAgentConfigured();
  const detail = await ctx.runQuery(internalApi["productivity/planner/queries"].getPlanByIdForUser, {
    userId: input.userId,
    id: input.planId,
  });
  ...
  const plannerContext = await ctx.runQuery(internalApi["productivity/planner/queries"].getPlannerAgentContext, {
    userId: input.userId,
  ...
  return await ctx.runMutation(internalApi["productivity/planner/mutations"].saveAgentReview, {
    userId: input.userId,
    planId: input.planId,
    agentThreadId: threadId,
...
async function replanWeeklyPlanForUser(
  ...
  return await ctx.runMutation(internalApi["productivity/planner/mutations"].performAgentReplan, {
    userId: input.userId,
    planId: input.planId,
    reason: result.object.reason,
    preserveLockedItems: result.object.preserveLockedItems,
...
async function ensurePlannerChatThreadForUser(ctx: ActionCtx, userId: string) {
  ...
  await ctx.runMutation(internalApi["productivity/planner/mutations"].setActivePlannerThread, {
    userId,
