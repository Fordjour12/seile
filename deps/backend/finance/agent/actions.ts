...
import {
  ...
  executeConfirmedFinanceProposal,
} from "../../ai/tools/finance";
...
export const sendFinanceAgentMessage = action({
  ...
  handler: async (ctx, args) => {
    ...
    const threadId = await ensureFinanceAgentThreadForUser(ctx, userId);
    const threadState = await financeCoachAgent.continueThread(ctx, {
      threadId,
      userId,
    });
    const context = await ctx.runQuery(
      internalApi["finance/agent/queries"].getFinanceAgentContext,
      { userId },
    );

    const structured = await financeCoachAgent.generateObject(
      ctx,
      { threadId },
      {
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      await ctx.runMutation(
        internalApi["finance/agent/mutations"].logConfirmedFinanceAction,
        {
          userId,
...
async function ensureFinanceAgentThreadForUser(ctx: ActionCtx, userId: string) {
  const state = await ctx.runMutation(
    internalApi["finance/agent/mutations"].getOrCreateFinanceAgentState,
    { userId },
  );
  ...
  await ctx.runMutation(
    internalApi["finance/agent/mutations"].setActiveFinanceThread,
    {
      userId,
